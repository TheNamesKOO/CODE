--// Made by NG. Script fully made by NG/Jeff Hoggleberg/convict.
--// Credits to trackvpn for the football magnets.
...
Library:Notify(
    {
	Title = "ViseHub",
	Content = "By saintsrevival and NG/Jeff Hoggleberg",
	Duration = 5
}
)
...
do
	...
	Magnets:OnChanged(
        function()
		ViseHub.Catching.FootballMagnets.Enabled = Options.Magnets.Value
	end
    )
	ShowMagnetHitbox:OnChanged(
        function()
		ViseHub.Catching.FootballMagnets.ShowMagnetHitbox = Options.Hitbox.Value
	end
    )
	VisualizeBallPath:OnChanged(
        function()
		ViseHub.Visuals.Visualizations.VisualizeBallPath.Enabled = Options.BallPath.Value
	end
    )
	VisualizeBallTrajectory:OnChanged(
        function()
		ViseHub.Visuals.Visualizations.VisualizeBallTrajectory.Enabled = Options.BallTrajectory.Value
	end
    )
	RemoveBallTrail:OnChanged(
        function()
		ViseHub.Visuals.Visualizations.RemoveBallTrail.Enabled = Options.RemoveBallTrail.Value
	end
    )
	ShowCatchEffects:OnChanged(
        function()
		ViseHub.Visuals.Visualizations.SelectCatchEffects.Enabled = Options.ShowCatchEffects.Value
	end
    )
	TackleAimbot:OnChanged(
        function()
		ViseHub.Defense.TackleAimbot.Enabled = Options.Tackle.Value
	end
    )
	QuickTP:OnChanged(
        function()
		ViseHub.Physics.QuickTeleport.Enabled = Options.QuickTP.Value
	end
    )
	AntiBlock:OnChanged(
        function()
		ViseHub.Physics.AntiBlock.Enabled = Options.AntiBlock.Value
	end
    )
	AntiJam:OnChanged(
        function()
		ViseHub.Physics.AntiJam.Enabled = Options.AntiJam.Value
	end
    )
	BigHead:OnChanged(
        function()
		ViseHub.Physics.BigHead.Enabled = Options.BigHead.Value
	end
    )
	AutoGuard:OnChanged(
        function()
		ViseHub.Automatics.AutoGuard.Enabled = Options.AutoGuard.Value
	end
    )
	AutoGetUp:OnChanged(
        function()
		ViseHub.Automatics.AutoGetUp.Enabled = Options.AutoGetUp.Value
	end
    )
	AutoRush:OnChanged(
        function()
		ViseHub.Automatics.AutoRush.Enabled = Options.AutoRush.Value
	end
    )
	AutoQB:OnChanged(
        function()
		ViseHub.Automatics.AutoQB.Enabled = Options.AutoQB.Value
	end
    )
	...
	local MagnetsPower =
		...
		Callback = function(Value)
			ViseHub.Catching.FootballMagnets.MagnetsPower = Value
		end
	}
    )
	local MagnetType =
		...
		Default = "Regular",
		Callback = function(Value)
			ViseHub.Catching.FootballMagnets.MagnetType = Value
		end
	}
    )
	local BigHeadSize =
		...
		Rounding = 1,
		Callback = function(Value)
			ViseHub.Physics.BigHead.Size = Value
		end
	}
    )
	local BigHeadTransparency =
		...
		Rounding = 2,
		Callback = function(Value)
			ViseHub.Physics.BigHead.Transparency = Value
		end
	}
    )
	local AutoGetUpDelay =
		...
		Rounding = 1,
		Callback = function(Value)
			ViseHub.Automatics.AutoGetUp.Delay = Value
		end
	}
    )
	local AutoRushDelay =
		...
		Rounding = 1,
		Callback = function(Value)
			ViseHub.Automatics.AutoRush.Delay = Value
		end
	}
    )
	...
	local JerseyNameInput =
		...
		Finished = true,
		Callback = function(Value)
			ViseHub.Player.SetJerseyName.Value = Value
		end
	}
    )
	local JerseyNumberInput =
		...
		Finished = true,
		Callback = function(Value)
			ViseHub.Player.SetJerseyNumber.Value = Value
		end
	}
    )
	...
	local AutoQBMethod =
		...
		Default = "Teleport",
		Callback = function(Value)
			ViseHub.Automatics.AutoQB.Method = Value
		end
	}
    )
end
...
Character.DescendantAdded:Connect(
    function(Descendant)
	if ViseHub.Physics.AntiBlock.Enabled and string.match(Descendant.Name, "FFmover") then
		Descendant:Destroy()
	end
end
)
				...
				if Player.Character then
					...
					for _, Child in next, Character:GetChildren() do
						if Child:IsA("Part") then
							if ViseHub.Physics.AntiJam.Enabled then
								Child.CanCollide = false
                            elseif not ViseHub.Physics.AntiJam.Enabled then
		...
		for _, Player in next, Players:GetPlayers() do
			if Player ~= LocalPlayer then
				...
				if Character then
					...
					if Head then
						if ViseHub.Physics.BigHead.Enabled then
							SizeX = Head.Size.X
							SizeY = Head.Size.Y
							SizeZ = Head.Size.Z
							OriginalTransparency = Head.Transparency
							Head.Size = Vector3.new(ViseHub.Physics.BigHead.Size, ViseHub.Physics.BigHead.Size, ViseHub.Physics.BigHead.Size)
							Head.Transparency = ViseHub.Physics.BigHead.Transparency
						elseif not ViseHub.Physics.BigHead.Enabled then
